...
router.get("/viewerreceiptview/:receiptId", (req, res) => {
  ...
  const sql = `
    ...
    FROM th_receipt
    LEFT JOIN th_user_project 
        ON th_receipt.user_project_fk = th_user_project.project_fk
    LEFT JOIN th_projects 
        ON th_user_project.project_fk = th_projects.project_id
    LEFT JOIN th_user 
        ON th_receipt.user_fk = th_user.member_pid
    LEFT JOIN th_installment 
        ON th_receipt.installment_fk = th_installment.installment_pk
    LEFT JOIN th_user_extra_charges 
        ON th_user_extra_charges.user_fk = th_user.member_pid
        AND th_user_extra_charges.user_project_fk = th_user_project.project_fk
    LEFT JOIN members_receipt_details AS mrd 
        ON mrd.receipt_id = th_receipt.receipt_pk
    
    WHERE th_receipt.receipt_pk = ?`;
...
router.post("/login", (req, res) => {
  ...
  const sql =
    "SELECT * FROM th_employee_details WHERE email_id = ? AND emp_epwd = ?";
